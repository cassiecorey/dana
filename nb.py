...
class NaiveBayes:
    ...
    def update_model(self, bow, label):
        ...
        for (token, count) in bow.iteritems():
            self.class_word_counts[label][token] += count
    ...
    def log_likelihood(self, bow, label, alpha):
        ...
        """
        return sum([ count * math.log(self.p_word_given_label_and_pseudocount(word, label, alpha))
    ...
    def log_prior(self, label):
        ...
        """
        return math.log(self.class_total_doc_counts[label] \
    ...
    def likelihood_ratio(self, word, alpha):
        ...
        """
        return self.p_word_given_label_and_pseudocount(word, POS_LABEL, alpha) \
                / self.p_word_given_label_and_pseudocount(word, NEG_LABEL, alpha)
...
def produce_hw1_results():
    ...
    # QUESTION 5.3
    alphas = [0.1, 1, 2,3, 4, 5, 6, 8, 10, 12, 15, 20, 50, 100]
    acc = map(lambda x: nb.evaluate_classifier_accuracy(x), alphas)
    print acc
    plot_pseudocount_vs_accuracy(alphas, acc)
...
def plot_pseudocount_vs_accuracy(pseudocounts, accuracies):
    """
    A function to plot pseudocounts vs. accuries. You may want to modify this function
